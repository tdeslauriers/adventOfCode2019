...
func getDist(s string) int {
	...
	r := []rune(s)
	sdist := string(r[1:])
	d, err := strconv.Atoi(sdist)
	if err != nil {
		log.Fatal("GetDist method cannot convert string to int.")
	}

	return d
}

func addNextCoords(dir string, dist int, coordlist [][]int) [][]int {

	// c := coordlist
	n := []int{coordlist[len(coordlist)-1][0], coordlist[len(coordlist)-1][1]}
...
func loadAllCoords(dirs []string, coords [][]int) [][]int {

	for i := 0; i < len(dirs); i++ {

		coords = addNextCoords(getDir(dirs[i]), getDist(dirs[i]), coords)
	}

	return coords
}

func findIntersections(p1, p2 [][]int) [][]int {
...
func findShortest(f [][]int) int {
	...
	for i := 0; i < len(f); i++ {
		...
		// requires absolute distance, no negatives
		x := f[i][0]
		if x < 0 {
			x = x * -1
		}

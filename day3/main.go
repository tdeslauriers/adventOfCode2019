...
func main() {
	...
	t2 := findLowest(findIntersections(loadAllCoords(p1, ta), loadAllCoords(p2, ta)))
...
func findIntersections(p1 [][]int, p2 [][]int) [][]int {
...
func findLowest(f [][]int) int {
	...
	for i := 0; i < len(f); i++ {
		...
		//manhatten distance requires absolutes, no negatives
		x := f[i][0]
		if x < 0 {
			x = x * -1
		}
